...
int main() {
	...
	int idxkropki = nazwapliku.find_last_of('.');
	string nazwabezrozsz = nazwapliku.substr(0, idxkropki);
	string rozsz = nazwapliku.substr(idxkropki);
	ofstream part1(nazwabezrozsz + "_part1" + rozsz);
	ofstream part2(nazwabezrozsz + "_part2" + rozsz);

	int polowarozm = calyplik.size() / 2;
	string polowa1 = calyplik.substr(0, polowarozm);
	string polowa2 = calyplik.substr(polowarozm); //substring od polowarozm do konca
	part1 << polowa1;
	part2 << polowa2;
}
